...
class CryptoLogoCacheService {
  ...
  static Future<void> initialize() async {
    ...
    try {
      ...
      if (cacheData != null && now.difference(cacheTime) < _cacheExpiry) {
        ...
        print('⚠️ Logo cache expired or empty, will refresh from API');
  ...
  static Future<String?> getLogoUrl(String symbol, {String? blockchain}) async {
    ...
    if (_logoCache.containsKey(cacheKey)) {
      print('✅ Logo cache hit for $cacheKey: ${_logoCache[cacheKey]}');
      return _logoCache[cacheKey];
    }
    
    // Try symbol only
    if (_logoCache.containsKey(symbol)) {
      print('✅ Logo cache hit for $symbol: ${_logoCache[symbol]}');
      return _logoCache[symbol];
    }
    
    print('❌ Logo cache miss for $symbol (blockchain: $blockchain)');
                ...
                : logoUrl != null && logoUrl!.startsWith('http')
                    ? CachedNetworkImage(
                        ...
                        errorWidget: (context, url, error) {
                          print('❌ Error loading cached logo from $url: $error');
                          return Icon(
