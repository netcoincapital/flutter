    ...
    LaxceAccessControl 
{
    ...
    /// @dev سهم treasury (15%)
    uint256 public constant TREASURY_ALLOCATION = 150_000_000 * 10**18;
    
    /// @dev سهم marketing (10%)
    uint256 public constant MARKETING_ALLOCATION = 100_000_000 * 10**18;
    
    /// @dev سهم liquidity bootstrap (15%)
    uint256 public constant LIQUIDITY_ALLOCATION = 150_000_000 * 10**18;
    ...
    function lockTokens(
        ...
        totalVotingPower = totalVotingPower.add(amount.mul(multiplier).div(Constants.BASIS_POINTS));
    ...
    function unlockTokens(uint256 lockIndex) external nonReentrant {
        ...
        totalVotingPower = totalVotingPower.sub(amount.mul(multiplier).div(Constants.BASIS_POINTS));
    ...
    function extendLock(
        ...
        totalVotingPower = totalVotingPower
            .sub(amount.mul(oldMultiplier).div(Constants.BASIS_POINTS))
            .add(amount.mul(newMultiplier).div(Constants.BASIS_POINTS));
    ...
    function distributeRewards() external onlyValidRole(ADMIN_ROLE) {
        ...
        uint256 annualReward = totalLocked.mul(rewardAPR).div(Constants.BASIS_POINTS);
    ...
    function getVotingPower(address user) external view returns (uint256) {
        ...
        for (uint256 i = 0; i < locks.length; i++) {
            ...
            votingPower = votingPower.add(
                lockInfo.amount.mul(lockInfo.multiplier).div(Constants.BASIS_POINTS)
            );
        }
    ...
    function _calculateLockMultiplier(uint256 duration) internal pure returns (uint256) {
        ...
        uint256 baseMultiplier = Constants.BASIS_POINTS;
        uint256 maxMultiplier = 4 * Constants.BASIS_POINTS;
    ...
    function _calculateUserRewards(address user) internal view returns (uint256) {
        ...
        uint256 userShare = userVotingPower.mul(Constants.BASIS_POINTS).div(totalVotingPower);
        uint256 totalRewards = revenueSharing[user].totalRewards;
        uint256 claimedRewards = revenueSharing[user].claimedRewards;
        
        return totalRewards.mul(userShare).div(Constants.BASIS_POINTS).sub(claimedRewards);
    }
    ...
    /**
     * @dev Override _update function required by both ERC20 and ERC20Votes
     * این تابع جایگزین _beforeTokenTransfer و _afterTokenTransfer شده
     */
    function _update(
        address from,
        address to,
