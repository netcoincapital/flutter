    ...
    LaxceAccessControl 
{
    ...
    function startMining(
        ...
        lpMining[msg.sender] = MiningInfo({
            ...
            rewardDebt: (amount * poolInfo.accRewardPerShare) / Constants.DECIMAL_BASE,
    ...
    function endMining() external nonReentrant {
        ...
        // اعمال multiplier
        uint256 rewards = (pending * info.multiplier) / Constants.FEE_BASE;
    ...
    function getPendingRewards(address user) external view returns (uint256) {
        ...
        if (block.timestamp > poolInfo.lastRewardTime && totalStaked > 0) {
            ...
            accRewardPerShare = accRewardPerShare + (
                (reward * Constants.DECIMAL_BASE) / totalStaked
            );
        }
        
        uint256 pending = (info.amount * accRewardPerShare) / Constants.DECIMAL_BASE - info.rewardDebt;
        
        return (pending * info.multiplier) / Constants.FEE_BASE;
    }
    ...
    function recordTradingVolume(
        ...
        // به‌روزرسانی آمار
        info.totalVolume = info.totalVolume + volume;
        ...
        // محاسبه reward
        uint256 baseReward = volume / 10000; // 0.01% از volume
        uint256 reward = (baseReward * newMultiplier) / Constants.FEE_BASE;
        
        if (reward > 0 && rewardPool >= reward) {
            info.rewardEarned = info.rewardEarned + reward;
        ...
        validAddress(to) 
    {
        ...
        poolInfo.totalLiquidity = poolInfo.totalLiquidity + amount;
        ...
        validAddress(from) 
    {
        ...
        if (withdrawFeeInfo.enabled) {
            fee = (amount * withdrawFeeInfo.feeRate) / Constants.FEE_BASE;
            
            if (fee > 0) {
                // انتقال fee
                _transfer(from, withdrawFeeInfo.feeRecipient, fee);
                withdrawFeeInfo.collectedFees = withdrawFeeInfo.collectedFees + fee;
        ...
        // به‌روزرسانی total liquidity
        poolInfo.totalLiquidity = poolInfo.totalLiquidity - burnAmount;
     ...
     * @return totalSupply_ کل عرضه توکن
     * @return totalLiquidity_ کل نقدینگی pool
     * @return totalStaked_ کل مقدار stake شده
     * @return rewardPool_ استخر پاداش
     * @return rewardPerSecond_ پاداش در ثانیه
     */
    ...
    function _updatePool() internal {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare + (
            (reward * Constants.DECIMAL_BASE) / totalStaked
        );
        ...
        returns (uint256) 
    {
        ...
        uint256 multiplier = baseMultiplier + (
            (duration * (maxMultiplier - baseMultiplier)) / maxDuration
        );
        ...
        returns (uint256) 
    {
        ...
        uint256 steps = totalVolume / volumeStep;
        uint256 multiplier = baseMultiplier + (steps * multiplierIncrease);
