...
library SqrtPriceMath {
        ...
        if (add) {
            uint256 quotient = (
                amount <= type(uint160).max
                    ? (amount << 96) / liquidity
                    : FullMath.mulDiv(amount, 2**96, liquidity)
            );
            ...
            uint256 quotient = (
                amount <= type(uint160).max
                    ? FullMath.mulDivRoundingUp(amount << 96, 1, liquidity)
                    : FullMath.mulDivRoundingUp(amount, 2**96, liquidity)
            );
    ...
    function getAmount1Delta(
        ...
        return
            roundUp
                ? FullMath.mulDivRoundingUp(liquidity, sqrtRatioBX96 - sqrtRatioAX96, 2**96)
